...
public class DeleteRecipeController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // Lấy thông tin công thức cần xóa từ request
        String recipeID = request.getParameter("recipeID");

        try {
            // Gọi phương thức trong DAO để xóa công thức
            RecipeOrganizeDAO recipeDAO = new RecipeOrganizeDAO();
            recipeDAO.deleteRecipe(recipeID);

            // Chuyển hướng đến trang quản lý công thức sau khi xóa thành công
            response.sendRedirect("RecipeManagementController");
        } catch (Exception ex) {
            // Xử lý thông báo lỗi nếu có
            String errorMessage = "Đã xảy ra lỗi khi xóa công thức: " + ex.getMessage();
